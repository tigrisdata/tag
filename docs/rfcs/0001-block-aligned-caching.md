...
- `cache.CachedObjectMeta` gains three fields:
  ...
  - `BlocksComplete bool` (`json:"blocks_complete,omitempty"`). Stamped when a populate wrote every block (the full-stream split) or a full assembly verified them (the promotion, below). A **hint, not an invariant** — blocks and meta evict/expire independently — that lets full-object serves and larger range serves skip the per-block probe pass; `false` (including entries written before the field existed) only means the probe-first path is used.
...
3. **Complete-entry optimistic path** (larger ranges when `BlocksComplete` is true and the first-slice staging reservation succeeds): read and recover the first requested slice into a pooled buffer **before committing the 206**, then write it and stream the exact remainder through the bounded recovery/salvage path. The first read is the presence check, so a warm range pays one payload read per covering block instead of a serial probe plus payload pass. A missing first slice still falls through cleanly before headers; a later eviction uses the existing bounded inline recovery and uncached remainder salvage. If staging declines, use the probe-first path.
4. **Probe-first path** (incomplete entries, or a staging-budget decline): probe each covering block with `BlockExistsErr` — a transient probe failure aborts rather than counting as missing, so a network blip can't masquerade as eviction — then fetch the missing ones and stream. At most `maxRangeBlockFanout` (32) absent blocks are fetched per request; a pathologically large client range with more absent blocks than that is served from a single upstream range GET instead of a fetch storm.
5. **Pipelined streaming**: multi-block spans use a bounded ordered prefetch window (at most `maxRangeBlockFanout`, currently 32 buffers). Independent cache reads begin concurrently, which lets blocks routed to different cache owners overlap; the write loop still commits block *i* before block *i+1*. Workers perform only the immutable cache read. A miss is recovered in write order, preserving the shared inline-fetch cap and first-error behavior. The largest two-to-32-buffer window that fits the staging budget is used; otherwise, and for single-block spans, serving degrades to the direct sequential stream.
...
- **Serve-staging**: the assembled-range buffer and complete-serve first-block buffer reserve their **actual sizes** against the same budget. Each buffer in the pipeline's bounded (at most 32) prefetch window reserves one block-sized slot, conservatively covering its range; staging is capped at **half** of the shared budget. A warm serve holds staging bytes for its whole (possibly multi-second) response; the cap guarantees populates a floor (the budget can't drop below `total − cap` through staging alone) so high-concurrency serving can't starve cold-miss populates and leave the working set cold — while populates can still use the whole budget when nothing is staging. The pipeline takes the largest two-to-32-buffer window that fits, then degrades to the sequential path if two buffers do not fit; it never fails a serve for staging pressure.
...
2. **Done** — block-aware range serve: probe-free assembled fast path for sub-block ranges, optimistic `BlocksComplete` path for larger ranges, conservative probe-first fallback with bounded fan-out, and partial-hit fetch (coalesced, concurrent).
