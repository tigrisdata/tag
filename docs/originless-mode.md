...
- **`DELETE`** invalidates the entry (not required for correctness — entries lapse by TTL — but explicit expiry gets prompt removal). A failed invalidation is surfaced, never acked: `500` on a single DELETE (retry it), a per-key `<Error>` in the multi-delete response (reported even in Quiet mode) — because the cache is the only store, a swallowed failure would leave the key serving until TTL. **Multi-delete** (`POST ?delete`, up to 1000 keys) works the same way; deleting an absent key is a success, as on S3.
...
Any query parameter — `?versionId`, `?partNumber`, `?tagging`, and the rest — selects a representation or an operation this mode does not implement, and answers `501` rather than doing something silently wrong. Two families of parameters are exempt because they select nothing: `x-id`, the no-op operation tag `aws-sdk-go-v2` appends to every request, and the auth parameters presigned URLs carry (`X-Amz-Signature`, `X-Amz-Credential`, and the rest) — a presigned request is served exactly like its header-signed twin, the signature ignored rather than evaluated, consistent with the trust model. Server-side copy (`X-Amz-Copy-Source`) is `501`:
