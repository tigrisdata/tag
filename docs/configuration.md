...
| `TAG_CACHE_COMPACTION_BPS`        | Shared source-read budget (bytes/second) for ocache background compaction + recompaction; protects serving reads on throughput-capped volumes. `0`/unset = unthrottled; set to a small fraction of the volume cap (e.g. `16777216` = 16 MiB/s on a 240 MB/s volume) | unthrottled              |
