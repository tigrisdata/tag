...
help:
	...
	@echo "  s3-test-originless     - Hermetic origin-less lifecycle: write, read, restart, full contract"
...
s3-test-originless: build
	@echo "Running origin-less lifecycle (write -> read -> restart -> full contract)..."
	@set -e; \
	B="originless-ci"; \
	...
	stop_tag() { \
		...
		timeout 40 bash -c 'while pgrep -fx "./$(BINARY_NAME)" >/dev/null 2>&1; do sleep 0.5; done' || \
			{ echo "TAG did not exit; RocksDB lock would collide"; exit 1; }; \
		sleep 1; \
	}; \
	trap 'pkill -fx "./$(BINARY_NAME)" 2>/dev/null || true' EXIT; \
	pkill -fx "./$(BINARY_NAME)" 2>/dev/null || true; \
	lsof -ti:$(TAG_LOCAL_HTTP_PORT) | xargs kill 2>/dev/null || true; sleep 1; \
	rm -rf $$DATA_DIR && mkdir -p $$DATA_DIR; \
	echo "--- Phase 1: origin-less TAG, populated by direct writes (production topology)"; \
	start_originless; \
	...
	echo "--- Phase 2: restart on the same cache directory, assert the full contract"; \
	stop_tag; \
	start_originless; \
